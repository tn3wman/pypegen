"""
ASME B16.11 Socket Weld Tee Generator using CadQuery.

This module creates socket weld tees (equal tees) per ASME B16.11 dimensions.
The tee has three ports: inlet (run-in), run (run-out), and branch.

Sources:
- https://www.ferrobend.com/dimensions/ansi-asme/pipe-fitting/b16.11-socket-weld-class-3000-tee/
- https://www.marcelpiping.com/asme-b16-11-forged-socket-weld-unequal-tee/
"""
...
@dataclass
class SocketWeldTeeDims:
    """ASME B16.11 Socket Weld Tee dimensions (all in mm).

    For an equal tee, the run and branch dimensions are the same.
    """

    center_to_end_run: float  # C - center to bottom of socket (run direction)
    center_to_end_branch: float  # M - center to bottom of socket (branch direction)
...
# ASME B16.11 Class 3000 Socket Weld Equal Tee dimension table
# Dimensions based on ASME B16.11 standard
# Note: Socket dimensions (B, J, C, G) match 90° elbow for same NPS
ASME_B1611_TEE_CLASS3000: dict[str, SocketWeldTeeDims] = {
    "1/8": SocketWeldTeeDims(
        center_to_end_run=9.5,
        center_to_end_branch=9.5,
    ...
    "1/4": SocketWeldTeeDims(
        center_to_end_run=11.0,
        center_to_end_branch=11.0,
    ...
    "3/8": SocketWeldTeeDims(
        center_to_end_run=13.5,
        center_to_end_branch=13.5,
    ...
    "1/2": SocketWeldTeeDims(
        center_to_end_run=15.5,
        center_to_end_branch=15.5,
    ...
    "3/4": SocketWeldTeeDims(
        center_to_end_run=19.0,
        center_to_end_branch=19.0,
    ...
    "1": SocketWeldTeeDims(
        center_to_end_run=22.5,
        center_to_end_branch=22.5,
    ...
    "1-1/4": SocketWeldTeeDims(
        center_to_end_run=27.0,
        center_to_end_branch=27.0,
    ...
    "1-1/2": SocketWeldTeeDims(
        center_to_end_run=32.0,
        center_to_end_branch=32.0,
    ...
    "2-1/2": SocketWeldTeeDims(
        center_to_end_run=41.0,
        center_to_end_branch=41.0,
    ...
    "4": SocketWeldTeeDims(
        center_to_end_run=66.5,
        center_to_end_branch=66.5,
...
def make_socket_weld_tee(
    nps: str,
    pressure_class: int = 3000,
) -> cq.Shape:
    """
    Create an ASME B16.11 socket weld equal tee.
    ...
    dims = ASME_B1611_TEE_CLASS3000.get(nps)
    if dims is None:
        raise ValueError(f"No ASME B16.11 Class {pressure_class} dimensions for NPS {nps}")

    # Get dimensions from ASME B16.11
    C_run = dims.center_to_end_run  # Center to end of socket (run)
    M_branch = dims.center_to_end_branch  # Center to end of socket (branch)
    ...
    # 3. A spherical intersection region at center
    ...
    run_bore = cq.Solid.makeCylinder(
        r_bore,
        C_run * 2 + 10,  # Extra length to ensure full cut
    ...
    branch_bore = cq.Solid.makeCylinder(
        r_bore,
        M_branch + 5,
        cq.Vector(0, -2, 0),  # Start slightly inside
        cq.Vector(0, 1, 0),
    )
    ...
    # Add socket counterbores at all three ends
    tee_shape = add_socket_counterbores(
        tee_shape,
...
def get_tee_dims(nps: str, pressure_class: int = 3000) -> SocketWeldTeeDims:
    """
    Get the dimensions for a socket weld tee.
    ...
    Returns:
        SocketWeldTeeDims with all dimensions
